...
<div id="<?= $_wrapper = strings::rand() ?>">
  ...
  ?>
    ...
    <div class="form-row border-bottom" data-id="<?= $dto->id ?>" data-type="<?= $dto->type ?>" data-person_id="<?= $dto->person_id ?>" data-name=<?= json_encode($dto->name, JSON_UNESCAPED_SLASHES) ?> data-mobile="<?= strings::CleanPhoneString($dto->mobile) ?>" data-email=<?= json_encode($dto->email, JSON_UNESCAPED_SLASHES) ?> data-fu_sms="<?= $dto->fu_sms ?>" data-has_reminder="<?= (int)$dto->reminder > 0 ? 'yes' : 'no' ?>">
  ...
  <?php if ($this->data->dto) {
    if ($this->data->dto->property_contact_id) { ?>
      <div class="form-row mt-5" data-contact_id="<?= $this->data->dto->property_contact_id ?>" data-inspect_diary_id="<?= $this->data->dto->id ?>">
        <div class="col">
